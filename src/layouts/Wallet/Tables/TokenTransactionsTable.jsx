...
import ECR20Abi from 'config/abi/erc20.json'
...
const TransactionsTable = ({ account, data, chainId, isTxLoading }) => {
  ...
  const getTokenDetails = async (list) => {
    const provider = new ethers.JsonRpcProvider(getRPCUrlById(chainId))
    const updatedItemsToDisplay = []
    setIsLoading(true)

    for (const x of list) {
      const tokenContract = new Contract(x.address, ECR20Abi, provider)
  ...
  return (
    <div className='grid grid-cols-1 w-full'>
      <Card title='ტოკენების ტრანზაქციები' titleBorder={true}>
        {!isTxLoading ? (
          <div>
            {data.status === 200 ? (
              <div>
                        ...
                        <Tbody>
                          {itemsToDisplay?.map((tx, index) => (
                            <BodyTr key={index}>
                              ...
                              <BodyTd isLast={index !== itemsToDisplay?.length - 1}>
                                <Typography className='whitespace-nowrap'>{(Number(tx.value) / 10 ** 18).toFixed(5)}</Typography>
                              </BodyTd>
                              <BodyTd isLast={index !== itemsToDisplay?.length - 1}>
                                <div className='flex flex-col'>
                                  <Typography>{tx.token_symbol}</Typography>
                                </div>
                              </BodyTd>
                            </BodyTr>
                          ))}
