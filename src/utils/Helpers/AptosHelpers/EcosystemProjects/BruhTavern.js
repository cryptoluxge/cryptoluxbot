  ...
  if (Object.keys(stakedBruhs).length > 0) {
    if (stakedBruhs.status === 200) {
      if (!stakedBruhs.data.hasOwnProperty('error_code')) {
        ...
        return { status: stakedBruhs.status, statusCode: 'info', staked: false, statusText: stakedBruhs.data.error_code === 'table_item_not_found' ? 'არ გაქვთ დასტეიკებული Bruh Bears' : stakedBruhs.data.error_code }
      }
    } else {
      return { status: stakedBruhs.status, statusCode: 'info', staked: false, statusText: stakedBruhs.data.error_code === 'table_item_not_found' ? 'არ გაქვთ დასტეიკებული Bruh Bears' : stakedBruhs.data.error_code }
    }
